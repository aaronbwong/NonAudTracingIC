...
for type_name in ["Type 1", "Type 2"]:
    probe_track = get_points_from_csv(csv_file, scale_to_um,marker_name=type_name)
    if len(probe_track) == 0:
        print(f"No data found for marker '{type_name}' in {csv_file}. Skipping...")
        continue
    ...
    output_csv = f"{base}_{type_name}_fit{ext}"
